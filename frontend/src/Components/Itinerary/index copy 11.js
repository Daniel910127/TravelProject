...
function Itinerary() {
  const [travelInfo, setTravelInfo] = useState({ travelList: [], startTime: Date.now(), dayCount: 0 });
  // const prevTravelInfo = usePrevious(travelInfo)
  const [days, setDays] = useState(['8/11', '8/12', '8/13']);
  // const [dayCount, setDayCount] = useState(3)

//每日開始時間startTime:{1:'28800'}
//旅遊開始日期startDate:@Date.now()
  ...
  useEffect(() => {
    const startDate = new Date(travelInfo.startTime);
                    ...
                    >
                      {
                        travelList.filter((item) => item.day === index + 1)
                          .map((item, index) => (<><Card item={item} index={index} key={item.id} />
                            <span>---------------------</span></>
